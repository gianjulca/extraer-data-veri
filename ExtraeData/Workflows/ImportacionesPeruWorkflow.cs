        ...
        public static async Task RunAsync(IPage page, string downloadsDir, VeritradeExcelReader reader, VeritradeSqlRepository repo)
        {
            ...
            async Task WaitTxtNandinaReadyAsync()
            {
                ...
                for (int attempt = 1; attempt <= 3; attempt++)
                {
                    try
                    {
                        ...
                        await partidaInputReady.WaitForAsync(new() { Timeout = 15000 });
                    ...
                    {
                        lastEx = ex;
                        Console.WriteLine($"[TXT-NANDINA-WARN] intento {attempt}/3 esperando #txtNandinaB habilitado: {ex.Message}");
            ...
            async Task ResetAndWaitReadyAsync()
            {
                ...
                for (int attempt = 1; attempt <= 3; attempt++)
                {
                    try
                    {
                        ...
                        await resetBtn.ClickAsync(new() { Force = true });
                        await page.WaitForTimeoutAsync(1500);
                    ...
                    {
                        lastEx = ex;
                        Console.WriteLine($"[RESET-WARN] intento {attempt}/3 falló: {ex.Message}");
            ...
            for (int i = 0; i < partidas.Length; i++)
            {
                ...
                /*  async Task TypePartidaAsync(string p)
                  {
                      for (int attempt = 1; attempt <= 3; attempt++)
                      {
                          ...
                          if (!string.IsNullOrWhiteSpace(current) &&
                              current.Contains(partida) &&
                              await suggestions.CountAsync() > 0)
                          {
                              break;
                          }

                          if (attempt == 3)
                              throw new Exception($"No se pudo escribir/mostrar autocomplete para '{partida}' en #txtNandinaB.");
                      }
                  }
                ...
                async Task TypePartidaAsync(string p)
                {
                    ...
                    for (int attempt = 1; attempt <= 4; attempt++)
                    {
                        try
                        {
                            ...
                            if (!string.IsNullOrWhiteSpace(current) && current.Contains(p))
                            {
                                ...
                                try
                                {
                                    await suggestions.First.WaitForAsync(new() { Timeout = 3000 });
                                }
                                catch
                                {
                                    // seguir reintentando
                            ...
                            lastEx = new Exception($"Autocomplete no visible para '{p}' en intento {attempt}/4.");
                        }
                        catch (Exception ex)
                        {
                            lastEx = ex;
                        ...
                        Console.WriteLine($"[TYPE-WARN] intento {attempt}/4 para partida={p} falló: {lastEx?.Message}");
                    ...
                    throw new Exception($"No se pudo escribir/mostrar autocomplete para '{p}' en #txtNandinaB. Último error: {lastEx?.Message}");
                }
                ...
                // ===== VALIDACION DUPLICIDAD (ANTES de descargar) =====
                ...
                for (int attemptDl = 1; attemptDl <= 2; attemptDl++)
                {
                    ...
                    catch (TimeoutException tex)
                    {
                        ...
                        // intento suave: volver a Mis Búsquedas y reintentar
                        try
                        {
                ...
                // “inyectar” los 3 campos a todas las filas sin romper tu modelo init-only:
                rows = rows.Select(r => new VeritradeRow
                {
                    // copiar TODO lo que ya trae r
        ...
        public static async Task RunAsync(IPage page, string downloadsDir, VeritradeExcelReader reader, VeritradeSqlRepository repo)
        {
            ...
            for (int i = 0; i < partidas.Length; i++)
            {
                ...
                for (int retryPartida = 1; retryPartida <= 2; retryPartida++)
                {
                    try
                    {
                        Console.WriteLine($"\n=== Partida {partida} ({i + 1}/{partidas.Length}) | intento {retryPartida}/2 ===");
                        ...
                        async Task TypePartidaAsync(string p)
                        {
                            ...
                            for (int attempt = 1; attempt <= 4; attempt++)
                            {
                                try
                                {
                                    ...
                                    await partidaInput.TypeAsync(p, new() { Delay = 80, Timeout = 5000 });
                                    await page.WaitForTimeoutAsync(900);
                    ...
                    return Array.from(sel.options || []).some(o => (o.textContent||'').includes(p));
                        ...
                        for (int attemptDl = 1; attemptDl <= 2; attemptDl++)
                        {
                            ...
                            catch (TimeoutException tex)
                            {
                                ...
                                try
                                {
                                    await page.GotoAsync("https://www.veritradecorp.com/es/mis-busquedas", 
                                        new() { Timeout = 120000, WaitUntil = WaitUntilState.DOMContentLoaded });
                                    await page.WaitForSelectorAsync("#btnBuscar", new() { Timeout = 120000 });
                                    await WaitTxtNandinaReadyAsync();
                                }
                                catch { }
                    ...
                    {
                        Console.WriteLine($"[RETRY-PERU] partida={partida} intento {retryPartida}/2 falló: {ex.Message}");
